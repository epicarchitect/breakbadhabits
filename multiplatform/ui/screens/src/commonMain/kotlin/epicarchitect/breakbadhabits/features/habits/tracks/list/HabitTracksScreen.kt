...
@Composable
fun HabitTracks(dependencies: HabitTracksDependencies) {
    ...
    val tracks by remember(dependencies) {
            ...
            items(currentTracks, key = { it.id }) { track ->
                Box(
                    modifier = Modifier
                        ...
                        .fillMaxWidth()
                        .clickable {
                            dependencies.navigation.openTrackEditing(track.id)
                        }
                ) {
                    Column(
                        ...
                        Text(
                            modifier = Modifier.padding(2.dp),
                            text = track.comment.ifBlank(dependencies.resources::habitTrackNoComment)
                        )
                    }
                }
            }
    ...
    Button(
        modifier = Modifier
            .padding(16.dp),
        onClick = {
            dependencies.navigation.openTrackCreation()
        },
        text = dependencies.resources.newEventButton(),
...
fun MonthOfYear.toEpic() = EpicMonth(year, month)
